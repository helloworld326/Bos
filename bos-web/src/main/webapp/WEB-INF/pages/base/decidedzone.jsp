...
<script type="text/javascript">
	...
	function doviewhighcharts(){
		$("#highchartsWindow").window("open");
		//页面加载完成后，动态创建图表
		$.post("subarea_findSubareasGroupByProvince", function(data){
			$("#test").highcharts({
				title: {
		            text: '区域分区分布图'
		        },
	...
	function doAssociations(){
		var f = $("#grid").datagrid("getSelections");
		if (f.length == 1) {
			$('#customerWindow').window('open');
			// 发送ajax请求数据
			var url_1 = "decidedzone_findNotAssociate";
			...
			var url_2 = "decidedzone_findAssociated";
			$.post(url_2, {"id" : f[0].id }, function(data){
			...
			$.messager.alert("提示信息", "请选中一个定区进行关联操作!", "warning");
		}
	}
	...
	var toolbar = [ {
		...
		id : 'button-viewcharts',
		text : '查看定区分布图',
		iconCls : 'icon-search',
		handler : doviewhighcharts
	}];
	// 定义列
	var columns = [ [ {
		field : 'id',
	...
	function doDblClickRow(index, data){
		$('#association_subarea').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "subarea_findByDecidedzoneId?decidedzoneId=" + data.id,
			columns : [ [{
				field : 'id',
				title : '分区编号',
				width : 120,
		...
		$('#association_customer').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "decidedzone_findAssociated?id=" + data.id, // 不是表单，所以要传递id值
			columns : [[{
				field : 'id',
				title : '客户编号',
				width : 120,
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="定区添加修改" id="addDecidedzoneWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="decidedzoneForm" action="decidedzone_add">
					...
					<tr>
						...
						<td>
							<input class="easyui-combobox" name="staff.id"  
    							data-options="valueField:'id',textField:'name',url:'staff_list'" />
						</td>
					</tr>
					<tr height="300">
						...
						<td>
							<table id="subareaGrid"  class="easyui-datagrid" border="false" style="width:300px;height:300px" 
								data-options="url:'subarea_list',fitColumns:true,singleSelect:false">
								<thead>  
							        <tr>  
	...
	<div class="easyui-window" title="关联客户窗口" id="customerWindow" collapsible="false" closed="true" minimizable="false" maximizable="false" style="top:20px;left:200px;width: 400px;height: 300px;">
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="customerForm" action="${pageContext.request.contextPath }/decidedzone_assignCustomer" method="post">
				<table class="table-edit" width="80%" align="center">
					<tr class="title">
					...
					<tr>
						...
						<td>
							<select id="associationSelect" name="cust_ids" multiple="multiple" size="10"></select>
						</td>
					</tr>
					<tr>
	...
	<!-- 查看highcharts窗口 -->
	<div class="easyui-window" title="区域分布图" id="highchartsWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
